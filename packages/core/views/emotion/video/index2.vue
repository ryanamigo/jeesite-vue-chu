  ...
  <div id="parent">
    ...
    <div class="child" style="color: #91f2f4">
      ...
      <div class="img-container" style="display: flex;">
        ...
        <div id="selfName" style="position: absolute; top: 50%;left: 5%; transform: translate(-50%, -50%); color: white; font-size: 20px;cursor: pointer;">
          视频录制
        </div>
    ...
    <div class="child">
      ...
      <div style="position: relative; display: inline-block;">
        ...
        <div id="infoCollection" style="position: absolute; top: 50%;left: 13%; transform: translate(-50%, -50%); color: white; font-size: 20px;cursor: pointer;">
          信息采集
        </div>
      </div>
      <!-- 照片 -->
      <div class="snapshot-container">
        ...
        <!-- 添加用于显示视频截图的画布 -->
        <canvas id="snapshot" width="160" height="150">近期照</canvas>
      </div>
      ...
      <div class="info-inputs" style="display: flex; flex-direction: column;color: #91f2f4;margin-left: 3%;">
        ...
        <div class="info-actions">
          <div>
            <button id="saveInfoButton" style="width: 75%;height: 40px;margin-left: 13%;margin-bottom: 8%;margin-top: 4%;"> 保存信息 </button>
            <div id="virtualBox1"></div>
          </div>
          <div>
            <button id="clearInfoButton" style="width: 75%;height: 40px;margin-left: 13%;"> 清空信息 </button>
          </div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  ...
  taskSel?.addEventListener('change', renderVideoStatus);
  const refresh = document.getElementById('refreshStausBtn');
  refresh?.addEventListener('click', renderVideoStatus);
  const selfName = getEl('selfName');
  ...
  const infoCollection = getEl('infoCollection');
  infoCollection?.addEventListener('click', () => {
    stopSnapshotInterval();
    setStyles('selfName', '', '');
  ...
  const snapshotBtn = getEl('snapshotButton');
  snapshotBtn?.addEventListener('click', handleSnapshot);

  const clearBtn = getEl('clearInfoButton');
